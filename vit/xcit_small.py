...
if __name__ == '__main__':
    """
    Para elegir modelos preentrenados disponibles en timm:
    avail_pretrained_models = timm.list_models(pretrained=True)
    cadena = "tiny"
    vits = [elem for elem in avail_pretrained_models if cadena in elem.lower()]

    tamano = "224"
    vits = [elem for elem in vits if tamano in elem]

    patches = "patch16" o "p16"
    vits = [elem for elem in vits if patches in elem]

    print(len(vits))
    #vits = sorted(vits, key=len)

    print(vits)
    """

    model_name = "xcit_small_12_p16_224.fb_in1k"
